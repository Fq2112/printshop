...
@section('title', __('admin.sidebar.head').': '.__('admin.tables.blog-gallery', ['param' => $blog->title]).' | '.env('APP_TITLE'))
@push('styles')
...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{__('admin.sidebar.blog-gallery')}}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="{{route('admin.dashboard')}}">Dashboard</a></div>
                <div class="breadcrumb-item">Data Master</div>
                <div class="breadcrumb-item">Blog</div>
                <div class="breadcrumb-item"><a href="{{route('table.blog.posts', ['q' => $blog->permalink])}}">Post</a></div>
                        ...
                        <div class="card-body">
                                ...
                                <table class="table table-striped" id="dt-buttons">
                                    ...
                                    @foreach($blog->getBlogGallery as $row)
                                        @php
                                            ...
                                            $url = route('detail.blog', ['author' => $blog->getAdmin->username,
                                            'y' => $date->format('Y'), 'm' => $date->format('m'), 'd' => $date->format('d'),
                                            'title' => $blog->permalink]);
                                        @endphp
                                        <tr>
                                            ...
                                            <td style="vertical-align: middle;">
                                                <div class="row lightgallery float-left mr-0">
                                                    ...
                                                    asset('storage/blog/'.$row->files) : $row->files}}"
                                                         data-sub-html="<h4>{{$row->files}}</h4><p>{{$blog->title}}</p>">
                                                        <a href="javascript:void(0)">
                                                            <img width="100" alt="Thumbnail" class="img-thumbnail"
                                                                 src="{{$row->type == 'photos' ? asset('storage/blog/'.
                                                                 $row->files) : asset('images/videoplaceholder.gif')}}">
                                                        </a>
                                                    </div>
                                                </div>
                                                @if($row->type == 'photos')
                                            ...
                                            <td style="vertical-align: middle" align="center">
                                                <a href="{{$url}}" data-placement="left" data-toggle="tooltip"
                                                   title="Preview" class="btn btn-info mr-1"><i class="fa fa-blog"></i>
                                                </a>
                                                <a href="{{route('delete.blog.galleries', ['blog_id' => encrypt($blog->id),
                                                'id' => encrypt($row->id)])}}" class="btn btn-danger delete-data"
                            ...
                            <div id="content2" style="display: none;">
                                <form id="form-blogGallery" method="post" enctype="multipart/form-data"
                                      action="{{route('update.blog.galleries', ['blog_id' => encrypt($blog->id)])}}">
                                    {{csrf_field()}}
                                    {{method_field('PUT')}}
                                        ...
                                        <span class="input-group-text fix-label-item" style="height: 2.25rem">
                                            <i class="fa fa-photo-video"></i></span>
                                                ...
                                                <select id="type" class="form-control selectpicker" title="-- Choose --"
                                                        name="type" data-live-search="true" multiple required>
                                    ...
                                    <div id="div-video" class="row form-group">
                                        <div class="col">
                ...
                table = $("#dt-buttons").DataTable({
                    ...
                    fnDrawCallback: function (oSettings) {
                        ...
                        $('.btn_massDelete').on("click", function () {
                            ...
                            $("#form-mass").attr("action", "{{route('massDelete.blog.galleries', ['blog_id' => encrypt($blog->id)])}}");
